  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-xl">
        <div>
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">Gestão de Rotas & Checkpoints</h2>
      ...
      {isLoading ? (
        ...
        <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-12 text-center text-slate-500 dark:text-slate-400">
          ...
          <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200">Nenhuma rota cadastrada</h3>
        ...
        <div className="grid grid-cols-1 gap-6">
          {routesList.map((route: any) => (
            <div
              key={route.id}
              className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-xl space-y-6"
            >
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
                <div>
                  <div className="flex items-center space-x-3">
                    <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100">{route.nome}</h3>
                    {route.isOrdered ? (
                  ...
                  <div className="flex items-center space-x-4 mt-2 text-xs text-slate-500 dark:text-slate-400">
                    <span>
                      Mínimo de Checkpoints: <strong className="text-slate-800 dark:text-slate-200">{route.qtdeMinimaCheckpoints}</strong>
                    </span>
                    <span>•</span>
                    <span>
                ...
                <div className="flex items-center space-x-3">
                  <button
                    ...
                    className="flex items-center space-x-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-blue-400 border border-slate-300 dark:border-slate-700 px-3.5 py-2 rounded-xl text-xs font-bold transition-all"
                  >
                ...
                {(!route.checkpoints || route.checkpoints.length === 0) ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {route.checkpoints.map((cp: any, idx: number) => (
                      <div
                        key={cp.id}
                        className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800/80 rounded-xl p-3.5 flex flex-col justify-between"
                      >
                        <div className="flex items-start justify-between">
                          <div className="flex items-center space-x-2">
                            ...
                            <h5 className="font-semibold text-sm text-slate-800 dark:text-slate-200">{cp.nome}</h5>
                          </div>
                        ...
                        <div className="mt-3 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 font-mono">
                          ...
                          <span className="text-[10px] bg-white dark:bg-slate-900 px-2 py-0.5 rounded text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-800">
                            QR Hash OK
                          </span>
                        </div>
                      </div>
      ...
      {showAddRouteModal && (
        <div className="fixed inset-0 z-50 bg-slate-50 dark:bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 w-full max-w-md shadow-2xl space-y-4">
            <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100">Cadastrar Nova Rota</h3>
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Nome da Rota</label>
                <input
                  type="text"
                  value={nomeRota}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Qtde Mín. Checkpoints</label>
                  <input
                    type="number"
                    min={1}
                    value={qtdeMin}
                ...
                <div className="flex items-center pt-5">
                  <label className="flex items-center space-x-2 text-xs font-medium text-slate-700 dark:text-slate-300 cursor-pointer">
                    <input
                      ...
                      className="rounded bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 text-blue-500 focus:ring-0"
                    />
      ...
      {addingCpRouteId && (
        <div className="fixed inset-0 z-50 bg-slate-50 dark:bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 w-full max-w-md shadow-2xl space-y-4">
            <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100">Adicionar Checkpoint à Rota</h3>
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Nome do Ponto</label>
                <input
                  type="text"
                  value={nomeCp}
