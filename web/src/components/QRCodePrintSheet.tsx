...
export const QRCodePrintSheet: React.FC<QRCodePrintSheetProps> = ({ route, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-50 dark:bg-slate-950/80 backdrop-blur-sm overflow-y-auto p-4 sm:p-6 flex flex-col items-center">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-4xl p-6 no-print mb-6 shadow-2xl">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">Imprimir Folha de QR Codes</h2>
          ...
          <div className="flex items-center space-x-3">
            <button
              onClick={handlePrint}
              className="flex items-center space-x-2 bg-blue-500 hover:bg-blue-400 text-white dark:text-slate-950 font-bold px-4 py-2.5 rounded-xl transition-all shadow-lg shadow-blue-500/20"
            >
            ...
            <button
              onClick={onClose}
              className="p-2.5 rounded-xl text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:text-slate-200 hover:bg-slate-100 dark:bg-slate-800 transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
      ...
      >
        <div className="border-b-2 border-slate-900 pb-4 mb-6 flex justify-between items-center">
          ...
          <div className="text-right">
            <span className="text-xs font-bold px-3 py-1 bg-white dark:bg-slate-900 text-white rounded-full uppercase">
              Patrol Sheet
            </span>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            {route.checkpoints.map((cp, idx) => (
              ...
              >
                ...
                <p className="text-[9px] text-slate-500 dark:text-slate-400 font-mono mt-2 break-all max-w-[200px]">
